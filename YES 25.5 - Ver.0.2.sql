...
CREATE TABLE `delivery_policy` (
	...
	`delivery_policy_min_order_amount`	decimal(10,2)	NOT NULL,
	`delivery_policy_is_member_only`	boolean	NOT NULL
);
...
CREATE TABLE `coupon_user` (
	...
	`user_coupon_type`	VARCHAR(255)	NULL	COMMENT 'ex) 생일, 웰컴'
);
...
ALTER TABLE `coupon_policy` ADD CONSTRAINT `PK_COUPON_POLICY` PRIMARY KEY (
	`coupon_policy_id`
);
...
ALTER TABLE `coupon` ADD CONSTRAINT `FK_coupon_policy_TO_coupon_1` FOREIGN KEY (
	`coupon_policy_id`
)
...
ALTER TABLE `coupon_policy_tag` ADD CONSTRAINT `FK_coupon_policy_TO_coupon_policy_tag_1` FOREIGN KEY (
	`coupon_policy_id`
)
REFERENCES `coupon_policy` (
	`coupon_policy_id`
...
ALTER TABLE `coupon_tag_category` ADD CONSTRAINT `FK_coupon_policy_TO_coupon_tag_category_1` FOREIGN KEY (
	`coupon_policy_id`
)
REFERENCES `coupon_policy` (
	`coupon_policy_id`
...
ALTER TABLE `coupon_policy_book` ADD CONSTRAINT `FK_coupon_policy_TO_coupon_policy_book_1` FOREIGN KEY (
	`coupon_policy_id`
)
REFERENCES `coupon_policy` (
	`coupon_policy_id`
...
ALTER TABLE `coupon_user` ADD CONSTRAINT `FK_coupon_TO_coupon_user_1` FOREIGN KEY (
	`coupon_id`
)
REFERENCES `coupon` (
	`coupon_id`
